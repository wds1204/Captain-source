...
public class EventBus {
    ...
    EventBus(EventBusBuilder builder) {
        //以event(即事件类)为key，以订阅列表(Subscription)为value，事件发送之后，
        // 在这里寻找订阅者,而Subscription又是一个CopyOnWriteArrayList，这是一个线程安全的容器。
        // 你们看会好奇，Subscription到底是什么，其实看下去就会了解的，
        // 现在先提前说下：Subscription是一个封装类，封装了订阅者、订阅方法这两个类。
        subscriptionsByEventType = new HashMap<Class<?>, CopyOnWriteArrayList<Subscription>>();

        //以订阅者类为key，以event事件类为value，在进行register或unregister操作的时候，会操作这个map。
